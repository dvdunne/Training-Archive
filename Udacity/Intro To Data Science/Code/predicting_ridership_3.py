...
# Add a weekday column to the data frame where 0 = weekend and 1 = weekday
def add_weekday_column(df):
    dates = df['DATEn']
    date_array = []

    for x in dates:
        date_object = datetime.datetime.strptime(x, "%Y-%m-%d").weekday()
        if date_object > 5:
            date_array.append(0)
        else:
            date_array.append(1)

    df['weekday'] =  np.array(date_array)
