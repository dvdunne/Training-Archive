...
def simpleGoodTuringProbs2(counts, countsOfCounts, confidenceLevel=1.96):
    """
    Given a dictionary mapping keys (species) to counts, returns a dictionary
    mapping those same species to their smoothed probabilities, according to
    Gale and Sampson's (1995/2001 reprint) "Simple Good-Turing" method of
    ...
    """
    # Gale and Sampson (1995/2001 reprint)
    totalCounts = float(6959156)   # N (G&S)  # Trigram
    totalCounts = float(6959156)   # N (G&S)  # Bigram
    totalCounts = float(6959156)   # N (G&S)  # Unigram
...
def read_countofCounts():
    import csv
    # reader = csv.reader(open("..\Data-Files\sgt\countofcount-fourgram.df.csv"))
    # reader = csv.reader(open("..\Data-Files\sgt\countofcount-trigram.df.csv"))
    # reader = csv.reader(open("..\Data-Files\sgt\countofcount-bigram.df.csv"))
    reader = csv.reader(open("..\Data-Files\sgt\countofcount-unigram.df.csv"))
    next(reader, None)  # skip header
    result = {}
    for row in reader:
        key = int(row[0])
        result[key] = int(row[1:][0])
    return result
...
def main():
    ...
    os.chdir("py/")
    print(os.getcwd() )
    # reader = csv.reader(open("..\Data-Files\dataframes\\fourgram.df.csv"))
    # reader = csv.reader(open("..\Data-Files\dataframes\\trigram.df.csv"))
    # reader = csv.reader(open("..\Data-Files\dataframes\\bigram.df.csv"))
    reader = csv.reader(open("..\Data-Files\dataframes\unigram.df.csv"))
    next(reader, None)  # skip header
    result = {}
    for row in reader:
        key = row[0]
        result[key] = int(row[0:][1])

    
    countOfCounts = read_countofCounts()
    sgtProb, P0 = simpleGoodTuringProbs2(result, countOfCounts)


    # sgtProb, P0 = simpleGoodTuringProbs(result)
    # print(sgtProb)

    # print type(sgtProb)

    # writer = csv.writer(open('..\Data-Files\sgt\\four-sgtProbs.csv', 'wb'))
    # writer = csv.writer(open('..\Data-Files\sgt\\tri-sgtProbs.csv', 'wb'))
    # writer = csv.writer(open('..\Data-Files\sgt\\bi-sgtProbs.csv', 'wb'))
    writer = csv.writer(open('..\Data-Files\sgt\\uni-sgtProbs.csv', 'wb'))
    for key, value in sgtProb.items():
